...
class RB(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_first = nn.Sequential(
            conv(n_feat, n_feat, kernel_size, bias=bias), act, conv(n_feat, n_feat, kernel_size, bias=bias))
...
class SAN(nn.Module):
    ...
    def __init__(self,
        ...
        # define tail module
        modules_tail = [Upsampler(conv, scale, n_feats, act=False), conv(n_feats, num_out_ch, kernel_size)]

        self.add_mean = MeanShift(img_range, rgb_mean, rgb_std, 1)
        self.non_local = Nonlocal_CA(
            in_feat=n_feats, inter_feat=n_feats // 8, reduction=8, sub_sample=False, bn_layer=False)
