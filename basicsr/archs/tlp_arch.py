...
class Block(nn.Module):
    ...
    def __init__(self, num_feat, res_scale=1):
        ...
        self.body = nn.Sequential(nn.Conv2d(num_feat, num_feat, 3, 1, 1), nn.SiLU())
...
class TLP(nn.Module):
    ...
    def __init__(self,
        ...
        self.cfa = nn.Sequential(
            nn.Conv2d(num_feat * 2, num_feat, 1, 1, 0), nn.Conv2d(num_feat, num_feat, 3, 1, 1), nn.SiLU())
    ...
    def forward(self, input):
        ...
        ua = self.am(out_fea)
        out_C = self.cfa(torch.cat([ua, out_fea], dim=1)) + out_fea
        out_B1 = self.IMDB1(out_C, ua)
