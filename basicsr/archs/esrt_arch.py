...
def defaultConv(inChannels, outChannels, kernelSize, bias=True):
    return nn.Conv2d(inChannels, outChannels, kernelSize, padding=(kernelSize // 2), bias=bias)
...
class FeedForward(nn.Module):
    ...
    def __init__(self, dim, hiddenDim, dropOut=0.):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(dim, hiddenDim), nn.GELU(), nn.Dropout(dropOut), nn.Linear(hiddenDim, dim), nn.Dropout(dropOut))
...
class EMHA(nn.Module):
    ...
    def __init__(self, inChannels, splitFactors=4, heads=8):
        ...
        self.reduction = nn.Conv1d(in_channels=inChannels, out_channels=inChannels // 2, kernel_size=1)
        self.attend = nn.Softmax(dim=-1)
        self.toQKV = nn.Linear(inChannels // 2, inChannels // 2 * 3, bias=False)
        self.expansion = nn.Conv1d(in_channels=inChannels // 2, out_channels=inChannels, kernel_size=1)
...
class EfficientTransformer(nn.Module):
    ...
    def __init__(self, inChannels, mlpDim=256, k=3, splitFactors=4, heads=8, dropOut=0.):
        ...
        self.emha = EMHA(inChannels=inChannels * k * k, splitFactors=splitFactors, heads=heads)
...
class ESRT(nn.Module):
    ...
    def __init__(self, hiddenDim=32, mlpDim=128, scaleFactor=2):
        ...
        self.path1 = nn.Sequential(
            BackBoneBlock(3, HPB, inChannel=hiddenDim, outChannel=hiddenDim, reScale=self.adaptiveWeight),
