...
class TEB_Residual(nn.Module):

    def __init__(self, num_feat, k_size=3):
        super(TEB_Residual, self).__init__()
...
class TEB(nn.Module):
    ...
    def __init__(self, num_feat, n_res):
        super(TEB, self).__init__()
        self.body = nn.Sequential(*[TEB_Residual(num_feat * 2**i) for i in range(n_res)])
...
class TEG(nn.Module):
    ...
    def __init__(self, num_feat, n_res, n_block):
        ...
        self.body = nn.Sequential(*[TEB(num_feat, n_res) for i in range(n_block)])
...
class TE(nn.Module):
    ...
    def __init__(self,
        ...
        for i in range(num_group):
            self.group.append(TEG(num_feat, num_res, num_block))
            self.dil.append(nn.Conv2d(num_feat, num_dil, 3, 1, 1, bias=False))

        self.conv_after_body = nn.Sequential(
            nn.Conv2d(num_feat + (num_group - 1) * num_dil, num_feat, 1, 1, 0, bias=False),
    ...
    def forward(self, x):
        ...
        for i in range(self.num_group - 1):
            out = self.group[i](out)
            d_out.append(self.dil[i](out))
        d_out.append(self.group[self.num_group - 1](out))
